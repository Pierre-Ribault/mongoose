#! /bin/bash
perf stat -x, -r 10 -e instructions,cycles,task-clock,page-faults,cpu-clock                         $@ #> /dev/null
...
perf stat -x, -r 10 -e dTLB-store-misses,dTLB-stores,iTLB-load-misses                               $@ #> /dev/null
perf stat -x, -r 10 -e dTLB-loads,iTLB-loads                                                        $@ #> /dev/null
